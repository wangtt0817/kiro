...
Version: 002.2 (Real Data - Fixed Logic)
Date: 2026-05-21

Fixed issues:
1. Removed random filtering for S3, S4, S5
2. Lowered thresholds further
3. Fixed S8 Monte Carlo logic
"""
...
# ============================================================
# Strategy Implementations (Fixed Logic)
# ============================================================
...
def s1_internal_arb(engine, price_history, market_lookup, dates):
    ...
    Adjusted: Look for sum < 0.98 (more realistic)
    """
    ...
    for date_idx in range(len(dates) - 1):
        ...
        # Adjusted: If sum < 0.98, there's an arb opportunity
        if total_yes < 0.98:
...
def s3_dixon_coles(engine, price_history, market_lookup, dates):
    ...
    Fixed: Removed random filtering, lowered threshold to 0.5%
    """
    result = StrategyResult(name="S3_DixonColes")
    teams = list(price_history.keys())
    
    # Lookback period for momentum
    lookback = 3  # Reduced from 5
    ...
    for date_idx in range(lookback, len(dates) - 1):
        ...
        for team in teams:
            ...
            price_3d_ago = get_price_at_index(price_history, team, date_idx - lookback, dates)
            if price_3d_ago is None:
                continue
            
            if price_3d_ago == 0:
                continue
            
            momentum = (entry - price_3d_ago) / price_3d_ago
            
            # Fair value based on momentum extrapolation
            fair_value = entry * (1 + momentum * 0.5)  # Increased from 0.3
...
def s4_elo_momentum(engine, price_history, market_lookup, dates):
    ...
    for date_idx in range(3, len(dates) - 1):
        ...
        for team in teams:
            ...
            fair_value = ma * 1.005  # Reduced from 1.01
            fair_value = max(0.005, min(0.995, fair_value))
...
def s5_inplay(engine, price_history, market_lookup, dates):
    ...
    Fixed: Removed random filtering, lowered threshold to 1.0 std devs
    """
    ...
    for date_idx in range(3, len(dates) - 1):
        ...
        for team in teams:
            ...
            # Detect overshoot: price > 1.0 std devs from mean (was 1.5)
            ...
            if entry > mean_price + 1.0 * std_price:
                # Overbought - sell
                direction = "BUY_NO"
                exit_price = get_price_at_index(price_history, team, date_idx + 1, dates)
                if exit_price is None:
            ...
            elif entry < mean_price - 1.0 * std_price:
                # Oversold - buy
                direction = "BUY_YES"
                exit_price = get_price_at_index(price_history, team, date_idx + 1, dates)
                if exit_price is None:
            ...
            if abs(edge) > 0.002:  # Lower threshold
...
def s6_market_making(engine, price_history, market_lookup, dates):
    ...
    Focuses on low-probability teams (< 10%).
    Adjusted: 10% threshold (was 5%)
    """
    result = StrategyResult(name="S6_MarketMaking")
    teams = list(price_history.keys())
    
    # Focus on teams with < 10% probability (adjusted from 5%)
    long_tail = [t for t in teams if market_lookup.get(t, {}).get('last_price', 1) < 0.10]
    
    for date_idx in range(len(dates) - 1):
        date_str = dates[date_idx]
        exit_date = dates[date_idx + 1]
        
        for team in long_tail[:10]:  # Max 10 positions
            entry = get_price_at_index(price_history, team, date_idx, dates)
            if entry is None:
                continue
            
            # Calculate spread
            half_spread = 0.015 if entry > 0.01 else 0.025
...
def s7_news(engine, price_history, market_lookup, dates):
    ...
    Detects significant price movements (>3% daily change) as news proxy.
    Adjusted: 3% threshold (was 5%)
    """
    ...
    for date_idx in range(1, len(dates) - 1):
        ...
        # 40% chance of major news per day
        ...
        for team in teams:
            ...
            # Detect news: >3% daily change (adjusted from 5%)
            daily_change = (entry - prev_price) / prev_price if prev_price > 0 else 0
            
            if abs(daily_change) > 0.03:
                # News detected - trade on continuation
                if daily_change > 0:
                    direction = "BUY_YES"
                    edge = daily_change * 0.3  # Expect 30% continuation
                else:
                    direction = "BUY_NO"
                    edge = abs(daily_change) * 0.3
...
def s8_monte_carlo(engine, price_history, market_lookup, dates):
    ...
    Fixed: Use actual price change instead of simulation
    """
    result = StrategyResult(name="S8_MonteCarlo")
    teams = list(price_history.keys())
    
    # Run simulation every 2 days
    for date_idx in range(3, len(dates) - 1, 2):
        date_str = dates[date_idx]
        exit_date = dates[min(date_idx + 2, len(dates) - 1)]
        ...
        for team in teams:
            ...
            # Fair value: extrapolate trend
            fair_value = entry * (1 + recent_change * 0.5)
            fair_value = max(0.005, min(0.995, fair_value))
            ...
            if abs(edge) > 0.005:
                ...
                exit_price = get_price_at_index(price_history, team, 
                                               min(date_idx + 2, len(dates) - 1), dates)
                if exit_price is None:
                    continue
                
                holding_days = min(2, len(dates) - 1 - date_idx)
...
def s9_closing_line(engine, price_history, market_lookup, dates):
    ...
    for date_idx in range(len(dates) - 1):
        ...
        for team in teams:
            # Only 20% of signals are actually tradeable
...
if __name__ == "__main__":
    ...
    # Save results
    output_path = f"{OUTPUT_DIR}/backtest_results_v002.2.json"
