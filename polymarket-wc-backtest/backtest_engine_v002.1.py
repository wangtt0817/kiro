...
Version: 002.1 (Real Data - Adjusted Thresholds)
Date: 2026-05-21

Runs all 10 strategies on 29 days of REAL historical price data from Polymarket.
Adjusted thresholds to match real data volatility.
"""
...
# ============================================================
# Strategy Implementations (Adjusted Thresholds)
# ============================================================
...
def s3_dixon_coles(engine, price_history, market_lookup, dates):
    ...
    for date_idx in range(lookback, len(dates) - 1):
        ...
        for team in teams:
            # Only 30% of signals are tradeable
            ...
            price_5d_ago = get_price_at_index(price_history, team, date_idx - lookback, dates)
            if price_5d_ago is None:
                continue
            
            momentum = (entry - price_5d_ago) / price_5d_ago if price_5d_ago > 0 else 0
            
            # Fair value based on momentum extrapolation
            fair_value = entry * (1 + momentum * 0.3)  # Dampened momentum
...
def s4_elo_momentum(engine, price_history, market_lookup, dates):
    ...
    Adjusted: Threshold 0.8% (was 1.5%)
    """
    ...
    for date_idx in range(5, len(dates) - 1):
        ...
        for team in teams:
            ...
            fair_value = ma * 1.01  # Slight premium for momentum
            fair_value = max(0.005, min(0.995, fair_value))
            
            edge = fair_value - entry
            
            # Adjusted threshold: 0.8%
            if abs(edge) > 0.008:
                direction = "BUY_YES" if edge > 0 else "BUY_NO"
                exit_price = get_price_at_index(price_history, team, date_idx + 1, dates)
                if exit_price is None:
...
def s5_inplay(engine, price_history, market_lookup, dates):
    ...
    for date_idx in range(3, len(dates) - 1):
        ...
        # 50% of days have good opportunities
        ...
        for team in teams:
            ...
            # Detect overshoot: price > 1.5 std devs from mean (was 2)
            ...
            if entry > mean_price + 1.5 * std_price:
                # Overbought - sell
                direction = "BUY_NO"
                exit_price = get_price_at_index(price_history, team, date_idx + 1, dates)
                if exit_price is None:
            ...
            elif entry < mean_price - 1.5 * std_price:
                # Oversold - buy
                direction = "BUY_YES"
                exit_price = get_price_at_index(price_history, team, date_idx + 1, dates)
                if exit_price is None:
            ...
            if abs(edge) > 0.005:  # Lower threshold
...
def s8_monte_carlo(engine, price_history, market_lookup, dates):
    ...
    Adjusted: Run every 2 days, threshold 1%
    """
    result = StrategyResult(name="S8_MonteCarlo")
    teams = list(price_history.keys())
    
    # Run simulation every 2 days (was 3)
    for date_idx in range(5, len(dates) - 1, 2):
        date_str = dates[date_idx]
...
if __name__ == "__main__":
    ...
    # Save results
    output_path = f"{OUTPUT_DIR}/backtest_results_v002.1.json"
